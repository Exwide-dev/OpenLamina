...
private:
     ...
     */
    [[nodiscard]] bool looksLikeDecoratedDoDecl();

    /**
     * @brief 判断当前位置是否为赋值语句（postfix 左值后跟 =）
     */
    [[nodiscard]] bool looksLikeAssignStmt();

    /**
     * @brief 解析 do 函数声明（含 with/make 或 postfix 装饰器）
     */
    ASTNode* parseDecoratedDoDecl();
     ...
     * @return 表达式节点
     */
    ExprNode* parsePostfixExpr(bool parse_do_suffix = true);
